...
class Video:
    ...
    send = (0,0)
    def __str__(self):
        return "标题：" +self.title + ' 分区' + str(self.partition) + '|' +  str(self.type_) + " 男:女={}:{}".format(self.send[0], self.send[1])

    def __repr__(self):
        return self.__str__()

    def get_list(self):
        return [self.title, self.partition, self.type_, self.send[0], self.send[1]]
...
def get_commenter(id):
    global g_proxies
    url = 'http://api.bilibili.com/x/v2/reply?type=1&oid={}&nohot=1&ps=49&pn={}'
    m_cnt = 0
    f_cnt = 0


    # 获取评论总页数
    path = url.format(id, 1)
    print(path)
    r = requests.get(path, headers=header, proxies=g_proxies)
    info = json.loads(r.text)
    if info['code'] == 0:
        total_page = info['data']['page']['count'] // 49 + 1

        for item in info['data']['replies']:
            sex = item['member']['sex']
            if sex =='男':
                m_cnt += 1
            elif sex == '女':
    ...
    idx = total_page
    cnt = 0
    while idx >= 2 and cnt < 1:
        path = url.format(id, idx)
        print(path)
        r = requests.get(path, headers=header, proxies=g_proxies)
        info = json.loads(r.text)
        if info['code'] == 0:
            idx -= 1
            cnt += 1
            if info['data'] is not None and info['data']['replies'] is not None:
            ...
            # g_proxies = get_proxy()
            exit(-1)
        elif info['code'] == 62002 or info['code']==62004:
            pass
        else:
            print(info['code'])
...
def craw_one(id, writer):
    ...
    if jt['code'] == 0:
        if jt['data'] is not None:
            ...
            view = jt['data']['stat']['view']

            v = Video()
            v.title = title
            v.partition = partition
            v.type_ = type_
            v.send = get_commenter(id)
...
def draw():
    ...
    bar = Bar()
    bar.add("男", x, y1, mark_point=["average"], is_stack=True)
    bar.add("女", x, y2, mark_line=["min", "max"], is_stack=True)
    bar.render("bar.html")
